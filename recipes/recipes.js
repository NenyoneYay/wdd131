console.log("Hello World! Recipes!");
...
function clickedSearch() {
    let userQuery = document.querySelector("#searchbar").value;
    let filterRecipes = recipes.filter(function(item){
        //Return name, description and tags
        return (
            item.name.toLowerCase().includes(userQuery.toLowerCase()) ||
            item.description.toLowerCase().includes(userQuery.toLowerCase()) ||
            item.tags.find(tag => tag.toLowerCase().includes(userQuery.toLowerCase()))
        );
    })
...
/* HTML for recipe-box for reference
        <div class="recipe-box">
            <img src="images/apple-crisp.jpg" alt="A bowl of apple crisp with ice cream on top">
            <div class="recipe-info">
